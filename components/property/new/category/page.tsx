...
export default function CategoryPage() {
    ...
    const scrollColumnStyle = 'flex flex-col w-full h-full space-y-2 overflow-scroll pr-1'
    const olStyle = 'flex flex-row p-3 justify-between items-center border rounded-md hover:bg-slate-50 hover:border-black'
        ...
        <div className='flex flex-row space-x-4 w-full h-[74%] p-4'>
            ...
            <div className={columnStyle}>
                <div>
                    ...
                    <p className='text-sm'>Seleziona la categoria che più si addice al tuo immobile</p>
                </div>
                <Input className='p-4' placeholder='Cerca per categoria...' />
                <div className={scrollColumnStyle}>
                    {
                        propertyCategoryTypeItems.map(
                            (item, i) => <ol
                                key={i}
                                className={olStyle}
                                onClick={() => {
            ...
            <div className={columnStyle}>
                <div>
                    ...
                    <p className='text-sm'>Seleziona la tipologia che più si addice al tuo immobile</p>
                </div>
                <Input className='p-4' placeholder='Cerca per tipologia...' />
                <div className={scrollColumnStyle}>
                    {
                        (propertyCategoryTypeItems[categorySelection].types != null)
                            ? propertyCategoryTypeItems[categorySelection].types!.map((item, i) => <ol
                                key={i}
                                className={olStyle}
                                onClick={() => {
                                    setTypeSeltion(i)

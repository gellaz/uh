...
export default function AppBar() {
    const notiStyle = 'flex w-9 h-9 rounded-md items-center justify-center hover:bg-slate-100';
    const iconSize = 20
    ...
    return (
        <div className='flex flex-row p-3 justify-between items-center border-b bg-background'>
            <div className='flex flex-row space-x-4 items-center'>
                {
                    usePathname().includes('message_center') || usePathname().includes('detail')
                        ? <Button
                            variant={'ghost'}
                            onClick={() => { router.back() }}
            ...
            <div className='flex flex-row space-x-4 justify-end w-1/5'>
                {
                    usePathname().includes('message_center') || usePathname().includes('new')
                        ? null
                ...
                {
                    usePathname().includes('notification_center') || usePathname().includes('new')
                        ? null
                ...
                {
                    usePathname().includes('profile') || usePathname().includes('new')
                        ? null
                        : <Avatar className={'flex w-9 h-9 rounded-full overflow-hidden items-center justify-center cursor-pointer'}>
